...
- **An honest run log** — after a manual run, the dashboard reports how many stories were actually verified as newly logged in the Sheet (`logged_count`), not just whether the HTTP call returned `200`. A `200` with zero newly-logged stories (e.g. no fresh headlines for an obscure topic) is shown as a distinct outcome from a real failure.
...
│   └── sheets_tool.py         # Google Sheets logging (gspread) + read_recent_stories() for the feed/cooldown
├── .github/
│   └── workflows/
...
- **Manual run reports `logged_count: 0`**: also not necessarily a bug — it means the pipeline ran but nothing new was verified in the Sheet, which happens when Serper/NewsAPI genuinely has no fresh headlines for that topic right now. Check the response's `"topics"` field to confirm what was actually searched, and the Slack channel / terminal's verbose CrewAI output if you suspect the Publisher or Logger agent failed silently instead.
